...
public class Program
{
    public static void Main(string[] args)
    {
        try
        {
            Console.WriteLine("Background process initialization started....");
            CronJob job = new CronJob();
            Console.WriteLine("--Processing Email Queueing....");
            job.GetProcessEmailQueueing();
            Console.WriteLine("--Fetching SAP Account with TOAS....");
            job.GetFetchAllSAPAccountWithTOAS();
            Console.WriteLine("--Synching Unit Acceptance/Deemed Acceptance Date....");
            job.GetSendTurnoverDateToSAP();
            Console.WriteLine("Background process successfully executed....");
        }
        catch (Exception ex)
        {
...
public class CronJob
{
    ...
    public void GetProcessEmailQueueing()
    {
                ...
                try
                {
                    ...
                    if (emailQueue != null)
                    {
                        foreach (var em in emailQueue)
                        {
                            ...
                            db.Database.ExecuteSqlCommand(sql, em.Id, true, DateTime.Now);
    ...
    public async void GetFetchAllSAPAccountWithTOAS()
    {
    ...
    public async void GetSendTurnoverDateToSAP()
    {
        using (WebAppEntities db = new WebAppEntities())
        {
            using (var dbContextTransaction = db.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    HttpClientHandler handler = new HttpClientHandler()
                    {
                        ...
                        //Credentials = new NetworkCredential("cantafd", "dr0w$$@P")
                    };
                    ...
                    using (var client = new HttpClient(handler))
                    {
                        ...
                        if (deemedAcceptance != null)
                        {
                            ...
                            foreach (var ua in deemedAcceptance)
                            {
                                ...
                                log.CreatedDate = DateTime.Now;
                                log.ObjectType = "GetSendTurnoverDateToSAP";
                                log.ContentDetail = string.Concat("[Console Application] | Company: ", ua.CompanyCode, "| Project: ", ua.ProjectCode, "| Unit Nos: ", ua.UnitNos, "| Deemed Acceptance Date: ", deemedAcceptanceDate);

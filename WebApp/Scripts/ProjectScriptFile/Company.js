app.controller("Company", ['$scope', '$http', '$window', '$timeout','$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {
    ...
    $scope.GetCompanyData = function () {
        ...
        urlData = '../api/Company/GetCompany';
        $http({
            method: 'GET',
            url: urlData,
            ...
            if (response.status === 200) {
                $scope.Company = response.data.COMPANYLIST;
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.Company, function (item) {
    ...
    $scope.EditCompanyData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };
    ...
    $scope.SaveCompany = function (data) {
        ...
        urlData = '../api/Company/SaveCompany';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
            ...
            document.body.style.cursor = 'default';
            if (response.data.Message === "Company Exists") {
                swal(
                    'Company Exists',
                    'Change Company Code',
                    'error'
    ...
    $scope.FetchCompany = function () {
        ...
        urlData = "../api/Company/SAPCompany";
        $http({
            method: 'POST',
            url: urlData,
            headers: headers,
    ...
    $scope.DeleteCompany = function (data) {
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeleteCompanyConfirmed(data);
            }
        });
    }

    $scope.DeleteCompanyConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        }
        url = "/api/Company/RemoveData";
        $http({
            method: 'POST',
            url: url,
            params: { 'Id': data.Id},

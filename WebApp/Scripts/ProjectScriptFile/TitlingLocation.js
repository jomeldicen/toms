app.controller("TitlingLocation", ['$scope', '$http', '$window', '$timeout', '$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {

    var tokenKey = 'accessToken';
    document.body.style.cursor = 'default';
    $scope.data = {};
    $scope.TitlingLocation = [];
    ...
    $scope.GetTitlingLocationData = function () {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
        ...
        urlData = '../api/TitlingLocation/GetTitlingLocation';
        $http({
            method: 'GET',
            url: urlData,
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.TitlingLocation, function (item) {
    ...
    $scope.StatusDataConfirmed = function (item, published) {
        ...
        url = '../api/TitlingLocation/UpdateStatus';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.EditTitlingLocationData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };

    $scope.SaveTitlingLocation = function (data) {
        $scope.$broadcast('show-errors-event');
        if ($scope.TitlingLocationForm.$invalid) {
            swal(
                'Error Message',
            ...
            $scope.SaveTitlingLocationConfirmed(data);
        }
    }

    $scope.SaveTitlingLocationConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        urlData = '../api/TitlingLocation/SaveTitlingLocation';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
            ...
            if (response.status === 200) {
                $scope.clearData();
                $scope.GetTitlingLocationData();
                $('#createTitlingLocationModal').modal('hide');
    ...
    $scope.DeleteDataConfirmed = function (item) {
        ...
        url = '../api/TitlingLocation/RemoveRecords';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.DeleteTitlingLocation = function (data) {
        swal({
            title: "Are you sure?",
            text: "Once deleted, you will not be able to recover this!",
            icon: "warning",
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeleteTitlingLocationConfirmed(data);
            }
        });
    }

    $scope.DeleteTitlingLocationConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        url = "/api/TitlingLocation/RemoveData";
        $http({
            method: 'POST',
            url: url,
            params: {'ID' : data.Id},

app.controller("Customer", ['$scope', '$http', '$window', '$timeout','$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {
    ...
    $scope.GetCustomerData = function () {
        ...
        urlData = '../api/Customer/GetCustomer';
        $http({
            method: 'GET',
            url: urlData,
            ...
            if (response.status === 200) {
                $scope.Customer = response.data.CustomerLIST;
    ...
    $scope.EditCustomerData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };
    ...
    $scope.SaveCustomer = function (data) {
        ...
        urlData = '../api/Customer/SaveCustomer';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
            ...
            document.body.style.cursor = 'default';
            if (response.data.Message === "Customer Exists") {
                swal(
                    'Customer Exists',
    ...
    $scope.FetchCustomer = function () {
        ...
        urlData = "../api/Customer/SAPCustomer";
        $http({
            method: 'POST',
            url: urlData,
            headers: headers,
    ...
    $scope.DeleteDataConfirmed = function (item) {
        ...
        document.body.style.cursor = 'wait';
        url = '../api/Customer/RemoveRecords';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.DeleteCustomer = function (data) {
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeleteCustomerConfirmed(data);
            }
        });
    }

    $scope.DeleteCustomerConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        }
        url = "/api/Customer/RemoveData";
        $http({
            method: 'POST',
            url: url,
            params: { 'Id': data.Id},

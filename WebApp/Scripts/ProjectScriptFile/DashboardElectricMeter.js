app.controller("DashboardElectricMeter", ['$scope', '$http', '$window', '$timeout', '$q', '$log', '$rootScope', function ($scope, $http, $window, $timeout, $q, $log, $rootScope) {

    var tokenKey = 'accessToken';
    document.body.style.cursor = 'default';
    $scope.data = { 'OptionIDs': [] };
    $scope.DashboardElectricMeter = [];
    $scope.ProjectName = '';
    $scope.loading = false;
    ...
    $scope.clearSearch = function () {
        $scope.data = { 'OptionIDs': [] };
        $scope.DashboardElectricMeter = null;
    ...
    $scope.search = function () {
        columnFilterId = ['8', '9', '10', '11', '12', '13', '14', '15'];
        ...
        $scope.GetDashboardElectricMeterData();
    };
    ...
    $scope.GetSearchData = function () {
        ...
        urlData = '../api/DashboardElectricMeter/GetSearchData';
        $http({
            method: 'GET',
            url: urlData,
    ...
    $scope.GetDashboardElectricMeterData = function () {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        urlData = '../api/DashboardElectricMeter/GetDashboardElectricMeter';
        $http({
            method: 'GET',
            url: urlData,
            ...
            if (response.status === 200) {
                $scope.DashboardElectricMeter = response.data.ELECTICMETERSTATUS;
    ...
    $scope.GetMainSummary = function () {
        ...
        urlData = '../api/DashboardElectricMeter/GetMainSummary';
        $http({
            method: 'GET',
            url: urlData,
            headers: headers,
    ...
    $scope.GetDetailSummary = function () {
        ...
        urlData = '../api/DashboardElectricMeter/GetDetailSummary';
        $http({
            method: 'GET',
            url: urlData,
            params: $scope.pagingInfo,
    ...
    $scope.FetchUpdatedElectricStatus = function () {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
    ...
    $scope.updatePlaceholder = function () {
        ...
        var columnFilterId = ['0', '1', '3', '4', '5', '6', '7'];
        if (columnFilterId.indexOf($scope.pagingInfo.searchcol) !== -1) {
            $scope.searchVisibleFilter = '0';
            //$scope.formatPlaceholder = 'search here...';
    ...
    $scope.linkRederict = function (UniqueHashKey) {
        ...
        window.location.href = '../Admin/ElectricMeter?flx10ms=' + UniqueHashKey;
    };
    ...
    $scope.ExportReport = function (control) {
        ...
        reportParameter.push({
            ...
            param2: $scope.pagingInfo.search2, // type of column in Summary ex 
        ...
        window.open(path + '?rep=k2rhh1234aeqxX0432ihfgh&contype=001x&json=' + json, '_blank');
    };

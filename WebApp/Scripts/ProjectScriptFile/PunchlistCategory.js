app.controller("PunchlistCategory", ['$scope', '$http', '$window', '$timeout', '$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {

    var tokenKey = 'accessToken';
    document.body.style.cursor = 'default';
    $scope.data = {};
    $scope.PunchlistCategory = [];
    ...
    $scope.search = function () {
        ...
        $scope.GetPunchlistCategoryData();
    };
    ...
    $scope.GetPunchlistCategoryData = function () {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
        ...
        urlData = '../api/PunchlistCategory/GetPunchlistCategory';
        $http({
            method: 'GET',
            url: urlData,
            ...
            if (response.status === 200) {
                $scope.PunchlistCategory = response.data.PunchlistCategoryLIST;
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.PunchlistCategory, function (item) {
    ...
    $scope.StatusDataConfirmed = function (item, published) {
        ...
        url = '../api/PunchlistCategory/UpdateStatus';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.EditPunchlistCategoryData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };

    $scope.SavePunchlistCategory = function (data) {
        $scope.$broadcast('show-errors-event');
        if ($scope.PunchlistCategoryForm.$invalid) {
            swal(
                'Error Message',
            ...
            $scope.SavePunchlistCategoryConfirmed(data);
        }
    }

    $scope.SavePunchlistCategoryConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        urlData = '../api/PunchlistCategory/SavePunchlistCategory';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
            ...
            if (response.status === 200) {
                $scope.clearData();
                $scope.GetPunchlistCategoryData();
                $('#createPunchlistCategoryModal').modal('hide');
    ...
    $scope.DeleteDataConfirmed = function (item) {
        ...
        url = '../api/PunchlistCategory/RemoveRecords';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.DeletePunchlistCategory = function (data) {
        swal({
            title: "Are you sure?",
            text: "Once deleted, you will not be able to recover this!",
            icon: "warning",
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeletePunchlistCategoryConfirmed(data);
            }
        });
    }

    $scope.DeletePunchlistCategoryConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        url = "/api/PunchlistCategory/RemoveData";
        $http({
            method: 'POST',
            url: url,
            params: {'ID' : data.Id},

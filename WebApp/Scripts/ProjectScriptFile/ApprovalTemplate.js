app.controller("ApprovalTemplate", ['$scope', '$http', '$window', '$timeout','$rootScope', function ($scope, $http, $window, $timeout, $rootScope) {

    var tokenKey = 'accessToken';
    document.body.style.cursor = 'default';
    $scope.data = {};
    $scope.ApprovalTemplate = [];
    ...
    $scope.GetApprovalTemplateData = function () {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
        ...
        urlData = '../api/ApprovalTemplate/GetApprovalTemplate';
        $http({
            method: 'GET',
            url: urlData,
            ...
            if (response.status === 200) {
                $scope.ApprovalTemplate = response.data.ApprovalTemplateLIST;
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.ApprovalTemplate, function (item) {
    ...
    $scope.StatusDataConfirmed = function (item, published) {
        ...
        url = '../api/ApprovalTemplate/UpdateStatus';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.EditApprovalTemplateData = function (data) {
        $scope.clearData();
        $scope.data = data;
    };

    $scope.SaveApprovalTemplate = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        urlData = '../api/ApprovalTemplate/SaveApprovalTemplate';
        $http({
            method: 'POST',
            url: urlData,
            data: data,
            ...
            if (response.status === 200) {
                $scope.clearData();
                $scope.GetApprovalTemplateData();
                $('#createApprovalTemplateModal').modal('hide');
                swal(
                    'Save Successful',
                    'ApprovalTemplate Saved Successfully',
                    'success'
                    );
            }
        }, function (response) {
            document.body.style.cursor = 'default';
            if (response.data.Message === "ApprovalTemplate Exists") {
                swal(
                    'ApprovalTemplate Exists',
                    'Change ApprovalTemplate Code',
                    'error'
                    );
            }            
        });
    ...
    // Start Fetch control
    $scope.FetchApprovalTemplate = function () {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        ...
        urlData = "../api/ApprovalTemplate/SAPApprovalTemplate";
        $http({
            method: 'POST',
            url: urlData,
            headers: headers,
            ...
            if (response.status === 200) {
                setTimeout(function () {
                    ...
                    $scope.clearData();
                    $scope.GetApprovalTemplateData();
                    $('#fetchApprovalTemplateModal').modal('hide');
    ...
    $scope.DeleteDataConfirmed = function (item) {
        ...
        document.body.style.cursor = 'wait';
        url = '../api/ApprovalTemplate/RemoveRecords';
        $http({
            method: 'POST',
            url: url,
            data: data,
    ...
    $scope.DeleteApprovalTemplate = function (data) {
        swal({
            title: "Are you sure?",
            text: "Once deleted, you will not be able to recover this!",
            icon: "warning",
        ...
        .then((willDelete) => {
            if (willDelete) {
                $scope.DeleteApprovalTemplateConfirmed(data);
            }
        });
    }

    $scope.DeleteApprovalTemplateConfirmed = function (data) {
        var token = sessionStorage.getItem(tokenKey);
        var headers = {};
        if (token) {
            headers.Authorization = 'Bearer ' + token;
        }
        url = "/api/ApprovalTemplate/RemoveData";
        $http({
            method: 'POST',
            url: url,
            params: { 'Id': data.Id},

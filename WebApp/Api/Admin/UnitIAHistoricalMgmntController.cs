...
{
    [Authorize]
    [RoutePrefix("api/UnitIAHistoricalMgmnt")]
    public class UnitIAHistoricalMgmntController : ApiController
    {
        private string PageUrl = "/Admin/UnitIAHistoricalMgmnt";
        private string ApiName = "Unit Historical Data Management";

        string timezone = "";

        private UnitIAHistoricalMgmntController()
        {
        ...
        [Route("GetUnitIAHistoricalMgmnt")]
        public async Task<IHttpActionResult> GetUnitIAHistoricalMgmnt([FromUri] SearchData item)
        {
            using (WebAppEntities db = new WebAppEntities())
            {
                try
                {
                    ...
                    turnoverstatus = await (from at in db.TurnoverStatusAcctTypes
                                            ...
                                            where ts.Published == true && op.Name == historicalInfo.AccountTypeCode
                                            select new
                                            {
                                                ts.Id,
                                                ts.Name,
                ...
                IEnumerable<CustomCustomerProfile> clientinfo = await (from csp in db.CustomerProfiles
                    ...
                    var data = new { CURUSER = user, EXCEPTIONDAYS = exceptionDays, TURNOVEROPTIONLIST = turnoverstatus, PUNCHLIST = punchlistcategory, HISTORICALINFO = historicalInfo, UNITHISTORICAL = unithistorical, CLIENTINFO = clientinfo, SYSPARAM = systemParameter, CONTROLS = permissionCtrl };
